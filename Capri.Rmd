...
comparecapri <- function(N, fpr, fnr, iters) {
  ...
  for(i in 1:iters) {
    ...
    troncomcc[i] <- (tp*tn-fp*fn)/(sqrt((tp+fp)*(tp+fn)*(tn+fp)*(tn+fn)))
    ...
    dpmcc[i] <- (tp*tn-fp*fn)/(sqrt((tp+fp)*(tp+fn)*(tn+fp)*(tn+fn)))
...
write.csv(caprimcc, file = "/Users/phillipnicol/Desktop/local_files/data/CPN_DP_DATA/caprimcc.csv")
write.csv(dpmcc, file = "/Users/phillipnicol/Desktop/local_files/data/CPN_DP_DATA/dpmcc.csv")
