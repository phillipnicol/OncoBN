...
comparemccbn <- function(N, fpr, fnr, iters) {
  ...
  returnlist <- list()
  returnlist[[1]] <- cbnprec
  returnlist[[2]] <- cbnrecall
  returnlist[[3]] <- dpprec
  returnlist[[4]] <- dprecall
  return(returnlist)
}
```
...
set.seed(9882771)
cbnprec <-  matrix(0, nrow = 100, ncol = length(eta))
cbnrecall <- matrix(0, nrow = 100, ncol = length(eta))
dpprec <- matrix(0, nrow = 100, ncol = length(eta))
dprecall <- matrix(0, nrow = 100, ncol = length(eta))

for(i in 1:length(eta)) {
  print("NEW")
  print(i)
  mylist <- comparemccbn(N = 400, fpr = eta[i], fnr = eta[i], iters = 100)
  cbnprec[,i] <- mylist[[1]]
  cbnrecall[,i] <- mylist[[2]]
  dpprec[,i] <- mylist[[3]]
  dprecall[,i] <- mylist[[4]] 
}
