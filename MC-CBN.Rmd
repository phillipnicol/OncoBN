...
```{r}
comparecapri <- function(G, theta, N, fpr, fnr, iters) {
  troncofp<- c(1:iters)
  troncofn <- c(1:iters)
  dpfn <- c(1:iters)
  dpfp <- c(1:iters)
  ...
  for(i in 1:iters) {
    ...
    troncofn[i] <- sum(troncomodel$model$capri_bic$adj.matrix$adj.matrix.fit - G_t  < 0)
    troncofp[i] <- sum(G_t - troncomodel$model$capri_bic$adj.matrix$adj.matrix.fit  < 0)
    
    out <- fitCPN(D, model = "DBN", k = 3, epsilon = min(colMeans(D))/2, verbose = FALSE)
  ...
  returnlist <- list()
  returnlist[[1]] <- troncofn
  returnlist[[2]] <- troncofp
  returnlist[[3]] <- dpfn
  returnlist[[4]] <- dpfp
  return(returnlist)
}
